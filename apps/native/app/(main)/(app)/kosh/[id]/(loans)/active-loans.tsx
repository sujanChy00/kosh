...
const ActiveLoanScreen = () => {
  ...
  const {
    ...
    trpc.loan.activeLoansByKosh.infiniteQueryOptions(
  ...
  if (isError) {
    return (
      <ErrorComponent
        refetch={refetch}
        message={error?.message ?? "Failed to load active loans."}
      />
    );
  }
  ...
  if (items.length === 0) {
    return (
      <EmptyComponent
        ...
        description="Active loans for this kosh will appear here."
      />
    );
  }
