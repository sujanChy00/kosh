...
export async function sendTransactionPinResetEmail({
  to,
  otp,
  name,
}: {
  ...
  const otpDigitsHtml = otp
    ? `<div class="otp-container">
        <div class="otp-title">Your 6-Digit Reset Code</div>
        <div class="otp-code">${otp}</div>
        <div class="otp-subtitle">Enter this code in the Kosh App to reset your transaction PIN</div>
       </div>`
    : "";
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset your Kosh Transaction PIN</title>
  <style>
    body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f4f5f7; margin: 0; padding: 24px; color: #1f2937; }
    .card { max-width: 520px; margin: 0 auto; background: #ffffff; padding: 36px 32px; border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.05); }
    .logo { font-size: 26px; font-weight: 800; color: #16a34a; text-decoration: none; display: inline-block; margin-bottom: 24px; letter-spacing: -0.5px; }
...
<body>
  <div class="card">
    ...
    <p>Namaste ${name ? name : "there"},</p>
    <p>We received a request to reset the transaction PIN for your <strong>Kosh App</strong> kosh. This code is valid for 10 minutes.</p>
    ${otpDigitsHtml}
    <p style="margin-top: 24px; font-size: 13px; color: #6b7280;">If you did not request this reset, you can safely ignore this email.</p>
    <div class="footer">
      &copy; ${new Date().getFullYear()} Kosh App. Built for community savings groups in Nepal.
    </div>
  </div>
  ...
  return sendMail({
    to,
    subject: `Reset your Kosh Transaction PIN ${otp ? `(Code: ${otp})` : ""}`,
    html,
  });
}
